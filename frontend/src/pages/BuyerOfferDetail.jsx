...
function BuyerOfferDetail() {
  ...
  usePageMeta({
    title: t('Offer'),
    description: t('View, counter, accept, or reject the offer the farmer sent on your demand.'),
  })
  ...
  if (detailStatus === 'loading') {
    return (
      <div className="p-8 text-center text-sm text-ink-500">{t("Loading offer…")}</div>
    )
  }
  if (detailStatus === 'failed') {
    return (
      <div className="p-8 text-sm text-rust-700">{detailError ? t(detailError) : t('Failed to load offer')}</div>
    )
  }
  ...
  return (
    <>
      <PageHeader
        eyebrow={t("Buying")}
        title={t("Offer")}
        description={`${t("For crop lot #")}${current.crop_lot_id} · ${t("Status")}: ${t(current.status)}`}
        back={{ to: '/buyer/offers', label: t('My offers') }}
      />
      <main className="mx-auto max-w-4xl px-4 py-6 sm:px-6 sm:py-8">

        <section className="ac-card p-5">
          <p className="text-sm text-ink-600">{t("Current price")}</p>
          <p className="font-display text-2xl font-bold text-ink-900">₹{current.current_price}</p>
          <p className="text-sm text-ink-600">
            {t("Quantity")}: <strong>{current.current_quantity}</strong>
          </p>
        </section>

        {canAct && (
          <section className="mt-4 ac-card p-5">
            <h2 className="font-display text-lg text-ink-900">{t("Your actions")}</h2>
        ...
        {!isOwn && (
          <div className="mt-4 rounded-card border border-honey-200 bg-honey-50 p-3 text-sm text-honey-800">
            {t("You are viewing this offer as a different buyer identity")} ({activeBuyer?.name || t('none')}).{' '}
            {t("Switch to")} <strong>{current.buyer?.name}</strong> {t("on the buyer dashboard to take actions.")}
          </div>
        )}
          ...
          <ol className="mt-3 space-y-2">
            {current.messages?.map((m) => (
              ...
              >
                ...
                {m.price && <p className="mt-1">{t("Price")}: <strong>₹{m.price}</strong></p>}
                {m.quantity && <p>{t("Qty")}: <strong>{m.quantity}</strong></p>}
